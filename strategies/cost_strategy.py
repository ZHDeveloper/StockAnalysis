...
from utils.stock_utils import stocks_dict, convert_stock_code
...
class CostStrategy:
    ...
    def analyze_stock(self, stock_code, cost_price):
        ...
        try:
            # 转换股票代码
            yf_stock_code = convert_stock_code(stock_code)
            self.logger.info(f"转换后的股票代码: {yf_stock_code}")
            ...
            if df is None or df.empty:
                self.logger.error(f"无法获取股票 {yf_stock_code} 的历史数据")
                return None
